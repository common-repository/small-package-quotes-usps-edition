...
if (!class_exists('EnUspsSpqLogsPackaging')) {
    class EnUspsSpqLogsPackaging
    {
        ...
        public function en_get_bins_packed($bin_response, $products_name_arr, $request)
        {
            ...
            $usps_box_types = ['UPMB', 'UMEB', 'UFLAT', 'customBoxes'];
            $updated_bins_arr = [];
            foreach ($bins_array as $details) {
                foreach ($usps_box_types as $type) {
                    if (isset($details->$type)) {
                        $updated_bins_arr[] = $this->appendProductTitle($details->$type, $products_name_arr, $request);
                    }
                }
            }

            $data = [
                'bins_array'        => $updated_bins_arr,
        ...
        public function appendProductTitle($bin_details, $products_name_arr, $request)
        {
            ...
            foreach ($bin_details as $k => $value) {
                if (isset($value->bin_data->id) && isset($value->items)) {
                    ...
                    foreach ($value->items as $key => $item_details) {
                        if (!empty($request['commdityDetails'])) {
                            foreach ($request['commdityDetails'] as $items) {
                                if (!empty($items)) {
                                    foreach ($items as $item) {
                                        if ($item['lineItemLength'] == $item_details->d && $item['lineItemWidth'] == $item_details->w && $item['lineItemHeight'] == $item_details->h) {
                                            $product_title = !empty($products_name_arr[$item_details->id]) ? $products_name_arr[$item_details->id] : '';
                                            $bin_details[$k]->items[$key]->product_name = $item['productName'];
                                        }
                                    }
                                }
                            }
